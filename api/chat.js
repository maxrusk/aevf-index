...
THE CORPUS (JSON)
Field guide for tasks: p = [price_low, price_base, price_high] USD per unit; r = [r_low, r_base, r_high]; fl = minimum reliability floor; tok = [input_tokens, output_tokens] per attempt; att = retry multiplier; tool and risk are USD per attempt; rev = [review minutes, reviewer role] (hourly rates in review_rates_hr); a = autonomy 0-5; s = substitutability 0-5; cf = confidence grade; src = source names.
IMPORTANT: each task's m field holds PRECOMPUTED base-case metrics at Claude Sonnet 5 with 50% cache hit, base R and base price: m = {C: total cost per attempt, tcevo, fcso, phi, ccr}. For any question about which tasks are viable, cheapest, most compressed, highest Phi, etc., READ m rather than recomputing; only redo arithmetic when the user changes an assumption (different model, R scenario, price scenario, review or risk multiplier), and then derive from the raw fields: C = att x token cost + tool + rev + 10% overhead + risk; Phi = r x price / C; FCSO = C / r; TCEVO = inference / r; CCR = price / C.
`;
