...
(function () {
  ...
  // ---- ask the index ----
  (function () {
    const log = $('chat-log'), form = $('ask-form'), input = $('ask-input');
    ...
    function add(role, text, pending) {
      ...
      who.className = 'who';
      who.textContent = role === 'user' ? 'You' : 'Index analyst';
    ...
    form.onsubmit = async e => {
      ...
      try {
        ...
        const ans = d.answer || ('Sorry, that failed: ' + (d.error || r.status));
        slot.parentElement.classList.remove('pending');
        slot.textContent = ans;
        if (d.answer) history.push({ role: 'assistant', content: ans });
      } catch (err) {
        slot.parentElement.classList.remove('pending');
        slot.textContent = 'Request failed: ' + err;
      }
      btn.disabled = false;
